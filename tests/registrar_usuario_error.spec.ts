...
test('muestra errores de validación si no se completan todos los campos', async ({ page }) => {
  await page.goto('http://localhost:3000/register');
  await page.getByRole('button', { name: 'Registrarse' }).click();
  await page.getByText('Nombre completoEl nombre debe tener al menos 2 caracteresEmailIngrese un email').click();
